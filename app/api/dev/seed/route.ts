  ...
  try {
    ...
    if (customers.length === 0 || barbers.length === 0 || services.length === 0 || stores.length === 0) {
      return NextResponse.json(
        {
          error: 'Devi avere almeno un cliente, un barbiere, un servizio e una sede prima di generare appuntamenti',
        },
        { status: 400 },
      );
    }
    ...
    const now = new Date();
    const appointments = [];
    // const statuses: Array<'CONFIRMED' | 'CANCELLED' | 'COMPLETED' | 'NO_SHOW'> = [
    //   'CONFIRMED',
    //   'CANCELLED',
    //   'COMPLETED',
    //   'NO_SHOW',
    ...
    return NextResponse.json({
      message: `Creati ${appointments.length} appuntamenti mock`,
      data: {
